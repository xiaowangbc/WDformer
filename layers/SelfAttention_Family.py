...
class DiffAttention(nn.Module):
    def __init__(self,d_model, n_heads, mask_flag=True, factor=5, scale=None, attention_dropout=0.1, output_attention=False, lambda_init = 0):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        O_concat = O_concat.transpose(1,2)
        if self.output_attention:
            return (O_concat.contiguous(), A)
        else:
            return (O_concat.contiguous(), None)
